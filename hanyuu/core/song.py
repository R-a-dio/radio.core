...
from .requests import requestable
...
class Song(object):
            ...
            if (key in ["lp", "id", "length", "filename", "metadata"]):
                ...
                with DictCursor() as cur:
                    if (key == "lp"):
                        ...
                        cur.execute(
                            "INSERT INTO eplay (`isong`, `dt`)"
                                "VALUES(%s, FROM_UNIXTIME(%s));",
                            (self.songid, int(value)),
                        )
                        if (self.afk):
                            cur.execute(
                                "UPDATE `tracks` SET `lastplayed`=FROM"
                                    "_UNIXTIME(%s) WHERE `id`=%s LIMIT 1;",
                                (self._lp, self.id),
                            )
                    elif (key == "length"):
                        # change database entries for length data
                        cur.execute(
                            "UPDATE `esong` SET `len`=%s WHERE id=%s",
                            (self.length, self.songid),
                        )
                    elif (key == "id"):
    ...
    def lp(self):
        ...
        """
        with Cursor() as cur:
            query = ("SELECT unix_timestamp(`dt`) AS ut FROM eplay,"
                " esong WHERE eplay.isong = esong.id AND esong.hash="
                "%s ORDER BY `dt` DESC LIMIT 1;")

            cur.execute(query, (self.digest,))
            for ut, in cur:
                return ut
            return None
...
album) AGAINST (%s) DESC LIMIT %s;",
    ...
    def nick(cls, nick, limit=5, tracks=False):
        with DictCursor() as cur:
            if (limit):
                cur.execute(
                    "SELECT esong.len AS len, esong.meta AS meta, tracks.id "
                        "AS trackid FROM tracks RIGHT JOIN esong ON tracks."
                        "hash = esong.hash JOIN efave ON efave.isong = esong."
                        "id JOIN enick ON efave.inick = enick.id WHERE enick."
                        "nick = %s LIMIT %s;",
                    (nick, limit),
                )
            else:
    ...
    def random(cls):
        ...
        with DictCursor() as cur:
            cur.execute(
                "SELECT * FROM `tracks` WHERE `usable`='1'"
            ...
            for row in cur:
                return cls(
                    ...
                    filename=os.path.join(config.music_directory,
                                          row['path']),
                )
